#!/usr/bin/env bash
# E3 arm E: arm B config + qazaqstantv fragment references switched to text_asr
...
set -uo pipefail
cd ~/krsl2speech
export ASAN_ROOT=$HOME/asan_canonical_asrfrag PYTHONPATH=. PYTHONUNBUFFERED=1 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
Q=$HOME/asan_canonical/score_quantiles.json
COMMON="--pretrained-unisign checkpoints/unisign/csl_stage1_weight.pth --selection-manifest $HOME/asan_canonical_asrfrag/selection_manifest.json --block-padding-mask --epochs 10 --seed 0"
declare -A ARGS=(
  [E]="--unisign-preprocess"
)
declare -A NAME=( [E]=upstream_asrfrag )
NEED_MIB=36000
QUEUE=(E)
declare -A GPU_PID=()
log(){ echo "[$(date +%H:%M:%S)] $*"; }
...
log "waiting for arm D to start training before competing for a GPU"
until grep -q "arm D training" output/e3d_scheduler.log 2>/dev/null; do sleep 120; done
log "arm D is training; E now waits for a free GPU"
declare -A ATTEMPTS=()
while [ ${#QUEUE[@]} -gt 0 ]; do
  arm=${QUEUE[0]}
  started=0
...
while pgrep -f "train_encoder_mt5.py.*e3_armE" >/dev/null; do sleep 600; done
log "E3 DONE"
for arm in E; do d=output/e3_arm${arm}_${NAME[$arm]}; echo "== $arm ${NAME[$arm]}"; grep -E "^Epoch" $d/train.log | tail -10; done
