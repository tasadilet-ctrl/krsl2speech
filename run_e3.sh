#!/usr/bin/env bash
# E3: input-handling ablation from clean-provenance init (Uni-Sign CSL weights + base mT5).
...
declare -A NAME=( [A]=legacy [C]=signspace [B]=upstream )
NEED_MIB=36000
QUEUE=(A C B)
declare -A GPU_PID=()
log(){ echo "[$(date +%H:%M:%S)] $*"; }
...
while pgrep -f "train_encoder_mt5.py.*e3_arm" >/dev/null; do sleep 600; done
log "E3 DONE"
for arm in A C B; do d=output/e3_arm${arm}_${NAME[$arm]}; echo "== $arm ${NAME[$arm]}"; grep -E "^Epoch" $d/train.log | tail -10; done
