#!/usr/bin/env bash
# E3 arm D: upstream Uni-Sign + per-frame hand-size scaling from clean-provenance init (Uni-Sign CSL weights + base mT5).
...
COMMON="--pretrained-unisign checkpoints/unisign/csl_stage1_weight.pth --selection-manifest $HOME/asan_canonical/selection_manifest.json --block-padding-mask --epochs 10 --seed 0"
declare -A ARGS=(
  [D]="--unisign-preprocess --unisign-hand-scale $HOME/asan_canonical/hand_ratio.json"
)
declare -A NAME=( [D]=upstream_handscale )
NEED_MIB=36000
QUEUE=(D)
declare -A GPU_PID=()
log(){ echo "[$(date +%H:%M:%S)] $*"; }
...
while pgrep -f "train_encoder_mt5.py.*e3_armD" >/dev/null; do sleep 600; done
log "E3 DONE"
for arm in D; do d=output/e3_arm${arm}_${NAME[$arm]}; echo "== $arm ${NAME[$arm]}"; grep -E "^Epoch" $d/train.log | tail -10; done
