...
class ProsodyTrainer:
    ...
    def __init__(self, config_path='configs/config.yaml', local_rank=0,
                 encoder_path=None, ckpt_path=None):
    ...
    def create_datasets(self):
        """Create prosody training datasets."""
        split = 0.9
        paths = self.config['paths']

        # Use Khabar KZ for now (needs prosody extracted)
    ...
    def train_epoch(self, train_loader, epoch):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.no_grad():
                encoder_out = self.encoder(kps)  # (B, T, d_model)
    ...
    def validate(self, val_loader):
        ...
        for batch in val_loader:
            ...
            recon_l1 = self.core._masked_l1(recon_kp, kps, input_lens).item()
