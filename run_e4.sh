#!/usr/bin/env bash
# E4: learning-rate balance screen. Arm-B config, NO alignment, ratio inverted:
# encoder 1e-4 / mT5 5e-5 (baseline is encoder 5e-5 / mT5 5e-4, takeoff 1/3).
...
launch(){
  local seed=$1 gpu=$2 dir=output/e4_lr_seed$1
  mkdir -p "$dir"; log "launch lr-screen seed $seed on GPU $gpu"
...
while pgrep -f "train_encoder_mt5.py.*e4_lr_seed" >/dev/null; do sleep 600; done
log "E4 DONE"
for s in 0 1 2; do echo "== seed $s"; grep -E "^Epoch" output/e4_lr_seed$s/train.log | tail -10; done
